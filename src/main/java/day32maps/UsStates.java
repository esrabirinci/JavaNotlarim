...
public enum UsStates {
    ...
    public static String getStatedNameFromAbbreviation(String abbreviation) {
        return
                Arrays.stream(values()).
                        filter(UsStates -> UsStates.getAbbreviation().
                                equals(abbreviation)).
                        findFirst().
                        get().
                        getStateName();
    ...
    public static String getStateNameFromCapital(String capital) {
        return
                Arrays.stream(values()).filter(t -> t.getCapital().equals(capital)).findFirst().get().getStateName();
    }

    //Example 1: Başkenti verilen state nin kısaltmasını ekrana yazdıran methodu oluşturunuz
